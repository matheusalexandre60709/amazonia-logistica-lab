    ...
    <section id="sobre" className="section-y bg-surface">
      <div className="container-x grid lg:grid-cols-[1fr_1.1fr] gap-12 items-start">
        <div>
          ...
          />
          <Reveal delay={0.1} className="mt-8 space-y-4 text-foreground/80 leading-relaxed">
            <p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {facts.map((f, i) => (
            <Reveal key={f.label} delay={0.05 * i}>
              <div className="h-full rounded-xl border border-border bg-card p-6 shadow-sm hover:shadow-elevated transition">
                ...
                <div className="mt-1 text-lg font-semibold text-foreground">{f.value}</div>
              </div>
            </Reveal>
          ))}
        </div>
